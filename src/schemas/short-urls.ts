...
// ---------------------------------------------------------------------------
// nevent_list_short_url_destinations (MCP 1.8.0)
// ---------------------------------------------------------------------------

/** Origin facet accepted by `GET /admin/short-url/destinations`. */
export const SHORT_URL_ORIGIN_VALUES = ['ALL', 'MANUAL', 'CAMPAIGN', 'ASSISTANT'] as const;

/**
 * Input schema for `nevent_list_short_url_destinations`.
 *
 * Lists short links grouped by canonical destination URL — one row per
 * destination instead of one per document. Pagination is over GROUPS.
 */
export const ListShortUrlDestinationsSchema = {
  origin: z
    .enum(SHORT_URL_ORIGIN_VALUES)
    .optional()
    .describe(
      'Filter by where the links came from. ALL (default) = every origin; ' +
      'MANUAL = links a human created; CAMPAIGN = links generated by campaign sends; ' +
      'ASSISTANT = system-managed links the chatbot pushed (read-only).'
    ),
  search: z
    .string()
    .optional()
    .describe(
      'Free-text search across destination URL, title, and short code. ' +
      'E.g. "festival" to find every destination for that event.'
    ),
  page: z
    .number()
    .int()
    .min(0)
    .optional()
    .describe('Zero-based page number over destination GROUPS. Default: 0 (first page).'),
  pageSize: z
    .number()
    .int()
    .min(1)
    .max(100)
    .optional()
    .describe('Number of destination groups per page (1-100). Default: 20.'),
};
