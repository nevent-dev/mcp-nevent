...
export function registerShortUrlTools(
  ...
  // -------------------------------------------------------------------------
  // Tool 10: nevent_list_short_url_destinations (MCP 1.8.0)
  // -------------------------------------------------------------------------
  server.tool(
    ...
    ListShortUrlDestinationsSchema,
    { title: 'List short URLs by destination', ...READ_ONLY_ANNOTATIONS_BASE },
    async (params) => {
      const denied = checkMode('nevent_list_short_url_destinations');
      if (denied) return err(denied);

      try {
        const result = await shortUrlClient.listDestinations({
